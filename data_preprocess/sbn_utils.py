...
def add_tuple_box(sbn, tuple_sbn, nodes_sbn):
...
def get_tuple(line):
    ...
    for i, cur_sbn in enumerate(simple_line):
        ...
        if len(cur_sbn) == 1:
            continue
        elif "Name" in cur_sbn[1] and include_quotes(cur_sbn[2]):
            # a,b = [],[]
            for nn in range(2, len(cur_sbn), 1):
                # a.append(cur_sbn[nn])
                # b.append([cur_sbn[0], cur_sbn[1], cur_sbn[index]])
            ...
            if end_index + 2 < len(cur_sbn):
                if "Name" in cur_sbn[end_index + 1] and include_quotes(cur_sbn[end_index + 2]):
    ...
    tuple_sbn, nodes_sbn = add_tuple_box(new_redundant_line, tuple_sbn, nodes_sbn)
    return tuple_sbn, nodes_sbn

...
from sbn_utils import get_sbn, sbn_string_to_list, between_quotes, list_to_file, file_to_list, is_number
...
def get_anonymize(new_clauses):
    ...
    for index_clause, cur_clause in enumerate(new_clauses):
        for idx, item in enumerate(cur_clause):
            ...
            #if cur_clause[idx - 1] == 'Quantity' and is_number(cur_clause[idx]) and not re.search("\+", cur_clause[idx]) and not re.search("\-", cur_clause[idx]):
